...
```mermaid
graph LR
    A[Frameworks] --> A1[Risk tiering]
    A --> A2[Control mapping]
    A --> A3[Intake & prioritisation]
    B[Projects] --> B1[Obligation classifier]
    B --> B2[Portfolio scorer]
    B --> B3[Control library explorer]
    C[Writing] --> C1[Articles]
    C --> C2[Drafts]
```
